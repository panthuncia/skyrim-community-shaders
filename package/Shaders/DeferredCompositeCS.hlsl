...
Texture2D<uint> Masks2Texture : register(t9);
...
[numthreads(8, 8, 1)] void main(uint3 dispatchID : SV_DispatchThreadID) {
	...
	// MASKS2 is a packed identity target. Vertex AO occupies the upper five
	// bits, preserving the old SSGI correction without consuming another MRT.
	float vertexAO = float(Masks2Texture[dispatchID.xy] >> 27) * (1.0 / 31.0);
