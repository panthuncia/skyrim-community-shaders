...
namespace
{
	...
	{
		return size >= required && version == CS_DX12_GRAPH_API_VERSION_1;
	}
}
...
struct DX12RenderRuntime::Pass
{
	...
	std::vector<std::pair<std::string, uint32_t>> accesses;
...
bool DX12RenderRuntime::CreateDeviceOnD3D11Adapter() noexcept
{
	...
	for (uint32_t i = 0; i < kCommandFrameCount; ++i) {
		if (FAILED(device12->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(allocators[i].put()))) ||
			FAILED(device12->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, allocators[i].get(), nullptr, IID_PPV_ARGS(commandLists[i].put()))) ||
			FAILED(commandLists[i]->Close()))
			return false;
	}
	return true;
}
...
{
	auto createPair = [&](winrt::com_ptr<ID3D12Fence>& fence12, winrt::com_ptr<ID3D11Fence>& fence11) {
		if (FAILED(device12->CreateFence(0, D3D12_FENCE_FLAG_SHARED, IID_PPV_ARGS(fence12.put()))))
			return false;
		HANDLE handle{};
	...
	if (!createPair(readyFence12, readyFence11) || !createPair(completeFence12, completeFence11)) {
...
void DX12RenderRuntime::SetDiagnostic(CSDX12Status status, std::string message) const noexcept
{
	...
	diagnostic.apiVersion = CS_DX12_GRAPH_API_VERSION_1;
...
CSDX12Status DX12RenderRuntime::Register(const CSDX12ContributorDesc* desc, CSDX12RegistrationHandle* out) noexcept
{
	...
	contributors.emplace(handle, Contributor{ handle, id, desc->kind, desc->userData, desc->build, desc->shutdown });
	*out = handle;
...
CSDX12Status DX12RenderRuntime::DeclarePass(CSDX12BuildHandle build, const CSDX12PassDesc* desc, CSDX12PassHandle* out) noexcept
{
	...
	for (uint32_t i = 0; i < desc->accessCount; ++i) {
		if (!desc->accesses || !desc->accesses[i].resourceId) return CS_DX12_E_INVALID_ARGUMENT;
		pass.accesses.emplace_back(desc->accesses[i].resourceId, desc->accesses[i].access);
	}
...
bool DX12RenderRuntime::Rebuild() noexcept
{
	...
	if (active) retired.emplace_back(nextFence.load(), std::move(active));
...
void DX12RenderRuntime::ExecuteDeferredEpoch(uint32_t width, uint32_t height) noexcept
{
	...
	if (!active)
		return;
	const uint32_t commandFrame = static_cast<uint32_t>(frameIndex % kCommandFrameCount);
	if (allocatorFence[commandFrame] && completeFence12->GetCompletedValue() < allocatorFence[commandFrame]) {
		SetDiagnostic(CS_DX12_E_RUNTIME_UNAVAILABLE, "Graph epoch skipped to avoid recycling an in-flight command allocator");
		return;
	}
	const uint64_t ready = nextFence.fetch_add(1);
	if (FAILED(context11->Signal(readyFence11.get(), ready)) || FAILED(graphicsQueue->Wait(readyFence12.get(), ready)))
		return;
	auto& allocator = allocators[commandFrame];
	auto& commandList = commandLists[commandFrame];
	if (FAILED(allocator->Reset()) || FAILED(commandList->Reset(allocator.get(), nullptr)))
		return;
	CSDX12FrameInfo frame{ sizeof(frame), CS_DX12_GRAPH_API_VERSION_1, frameIndex++, active->id, width, height };
	for (const auto& pass : active->passes) {
		CSDX12Status status = CS_DX12_E_CALLBACK_FAILED;
		try { status = pass.execute(pass.userData, commandList.get(), &frame); } catch (...) { status = CS_DX12_E_CALLBACK_FAILED; }
		if (status != CS_DX12_OK) SetDiagnostic(status, "Execution callback failed: " + pass.id);
	}
	if (FAILED(commandList->Close()))
		return;
	ID3D12CommandList* lists[]{ commandList.get() };
	graphicsQueue->ExecuteCommandLists(1, lists);
	const uint64_t complete = nextFence.fetch_add(1);
	if (SUCCEEDED(graphicsQueue->Signal(completeFence12.get(), complete))) {
		allocatorFence[commandFrame] = complete;
...
void DX12RenderRuntime::RetireCompleted() noexcept
{
	...
	while (!retired.empty() && retired.front().first <= completed) retired.pop_front();
}
...
void DX12RenderRuntime::Shutdown() noexcept
{
	...
	if (graphicsQueue && completeFence12) {
		const auto value = nextFence.fetch_add(1);
		graphicsQueue->Signal(completeFence12.get(), value);
		if (context11 && completeFence11) context11->Wait(completeFence11.get(), value);
	}
	std::unique_lock lock(registryMutex);
	for (auto& [_, contributor] : contributors) if (contributor.shutdown) contributor.shutdown(contributor.userData, active ? active->id : 0);
	contributors.clear(); active.reset(); retired.clear();
	renderGraph.reset();
	for (uint32_t i = 0; i < kCommandFrameCount; ++i) { commandLists[i] = nullptr; allocators[i] = nullptr; allocatorFence[i] = 0; }
	readyFence11 = nullptr; readyFence12 = nullptr;
	completeFence11 = nullptr; completeFence12 = nullptr; copyQueue = nullptr; computeQueue = nullptr;
...
CSDX12Status DX12RenderRuntime::GetRuntimeInfo(CSDX12RuntimeInfo* out) const noexcept
{
	...
	*out = { sizeof(*out), CS_DX12_GRAPH_API_VERSION_1, available.load() ? 1u : 0u, 0u,
		active ? active->id : 0u, completeFence12 ? completeFence12->GetCompletedValue() : 0u, 1u, 1u };
	return CS_DX12_OK;
}

CSDX12Status DX12RenderRuntime::GetLastDiagnostic(CSDX12Diagnostic* out) const noexcept
{
	if (!out || out->structSize < sizeof(*out)) return CS_DX12_E_INVALID_ARGUMENT;
	std::scoped_lock lock(diagnosticMutex); *out = diagnostic; return CS_DX12_OK;
}
...
namespace
{
	...
	CSDX12Status CS_DX12_GRAPH_CALL ApiDiagnostic(CSDX12Diagnostic* d) { return DX12RenderRuntime::Get().GetLastDiagnostic(d); }
}
...
extern "C" CSDX12Status CS_DX12_GRAPH_CALL CS_GetDX12GraphAPI(uint32_t version, CSDX12GraphAPI* out)
{
	...
	if (version != CS_DX12_GRAPH_API_VERSION_1) {
		return CS_DX12_E_UNSUPPORTED_VERSION;
	}
	*out = { sizeof(*out), version, &ApiRuntimeInfo, &ApiDevice, &ApiRegister, &ApiUnregister, &ApiResource, &ApiPass, &ApiRebuild, &ApiDiagnostic };
	return CS_DX12_OK;
}
